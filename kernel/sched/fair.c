...
static struct task_struct *detach_specific_task(struct lb_env *env, struct task_struct *pm);
#else
static struct task_struct *detach_specific_task(struct lb_env *env, struct task_struct *pm)
{
	return NULL;
...
/*
 * detach_specific_task tries to detach a specific task.
 * Returns the task if successful and NULL otherwise.
 * Called with runqueue locked.
 */
static struct task_struct *detach_specific_task(struct lb_env *env, struct task_struct *pm)
{
	struct task_struct *p, *n;
	...
	list_for_each_entry_safe(p, n, &env->src_rq->cfs_tasks, se.group_node) {
		...
		detach_task(p, env);
		/*
		 * Right now, this is only the third place detach_task()
		 * is called, so we can safely collect detach_task()
		 * stats here rather than inside detach_task().
		 */
		schedstat_inc(env->sd, lb_gained[env->idle]);
		return p;
	}
